package Stream_API.Reduce;

import java.util.Arrays;
import java.util.List;
import java.util.function.Predicate;

public class Match {
    public static void main(String[] args) {
        Aluno a1 = new Aluno("Ana", 7.1);
        Aluno a2 = new Aluno("Luna", 6.1);
        Aluno a3 = new Aluno("Gabi", 8.1);
        Aluno a4 = new Aluno("Ruan", 10);

        List<Aluno> alunos = Arrays.asList(a1, a2, a3, a4);

        Predicate<Aluno> aprovado = a -> a.nota >= 7;
        Predicate<Aluno> reprovado = aprovado.negate();

        //allMatch retorna true se todos os elementos do fluxo correspondem ao predicado fornecido.
        System.out.println(alunos.stream().allMatch(aprovado));//false
        //anyMatch retorna true se algum elemento do fluxo for verdadeiro.
        System.out.println(alunos.stream().anyMatch(aprovado));//true
        //noneMatch retorna true se nenhum dos elementos do fluxo corresponde ao predicado fornecido.
        System.out.println(alunos.stream().noneMatch(aprovado));//false
        System.out.println(alunos.stream().noneMatch(reprovado));//false
    }
}
